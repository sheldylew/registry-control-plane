...
const CACHE_PREFIX = "registry-control-plane:maintenance:v1";
const memoryCache = new Map();

function cacheKey(page) {
  return `${CACHE_PREFIX}:page:${page}`;
}
...
function writeCachedPayload(key, payload) {
  ...
  try {
    ...
    // Browser storage can be unavailable or quota-limited. Memory cache still covers this tab.
  }
  return cached;
}
    ...
    <div className="space-y-6">
      <Panel className="p-6">
        <PanelHeader
          ...
          description="The page is checking for a cached dashboard snapshot while a fresh status update runs in the background."
        />
      </Panel>
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        {["Registry status", "Storage usage", "Manifest summaries", "Refreshed in 24h", "Last job"].map((label) => (
          <article key={label} className="rounded-lg border border-white/10 bg-slate-900/80 p-6 shadow-sm shadow-slate-950/20">
...
export default function MaintenanceDashboardClient({ initialPage, timeZone }) {
  const [payload, setPayload] = useState(null);
  const [cachedAt, setCachedAt] = useState(null);
  const [refreshing, setRefreshing] = useState(false);
  const [refreshError, setRefreshError] = useState("");
  const key = cacheKey(initialPage);

  const refreshPayload = useCallback(async () => {
    setRefreshing(true);
    setRefreshError("");
    try {
      const response = await fetch(`/api/admin/maintenance?page=${initialPage}`, {
        cache: "no-store",
      });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(body.detail || "Failed to refresh maintenance status.");
      }
      const cached = writeCachedPayload(key, body);
      setPayload(cached.payload);
      setCachedAt(cached.cachedAt);
    } catch (error) {
      setRefreshError(error instanceof Error ? error.message : "Failed to refresh maintenance status.");
    } finally {
      setRefreshing(false);
    }
  }, [initialPage, key]);
  ...
  if (!payload) {
    return (
      <div className="space-y-6">
        {refreshError ? <Alert tone="rose">{refreshError}</Alert> : null}
        <LoadingShell />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {cachedAt || refreshing ? (
        <div className="flex flex-wrap items-center gap-3 rounded-lg border border-white/10 bg-slate-950/50 px-4 py-3 text-sm text-slate-300">
          {cachedAt ? <span>Last dashboard refresh {formatDateTime(cachedAt, { timeZone })}.</span> : null}
          {refreshing ? <Badge tone="cyan">Refreshing</Badge> : null}
        </div>
      ) : null}
      ...
      <MaintenancePanel logRetentionDays={payload.log_retention_days} onAfterMutation={refreshPayload} />
